...
const CodeConverterPage = () => {
  ...
  const [editorLanguages, setEditorLanguages] = useState(null);
  const [finalResponse, setFinalResponse] = useState("");
  const [isLoading, setIsLoading] = useState(false); // Initialize as loading
  const outputString = finalResponse;
  const [value, setValue] = useState(outputString);

  const exportFileData = () => {
    return outputString; // Replace this with your actual data
  };

  const payLoad = {
    from_laguage: editorLanguages?.selectedSourceLanguage.sourceLanguageName,
    to_language: editorLanguages?.selectedTargetLanguage.targetLanguageName,
    code: fileData,
  };
  ...
  const convertCode = async (payLoad) => {
    try {
      const response = await axios.post(
        `http://localhost:5000/codeconversion/convert-code`,
        payLoad
      );
      setIsLoading(false);
      ...
      <div className="flex flex-wrap justify-center items-center font-bold text-4xl p-10">
        <a href="#">
          <img
            ...
            src={editorLanguages?.selectedSourceLanguage.imageURL}
            alt="JavaScript Image"
          />
        </a>
        <div className="font-bold text-2xl mb-2 pt-2 pr-4">
          {editorLanguages?.selectedSourceLanguage.sourceLanguageName}
        </div>
        ...
        <a href="#">
          <img
            ...
            src={editorLanguages?.selectedTargetLanguage.imageURL}
            // alt="JavaScript Image"
          />
        </a>
        <div className="font-bold text-2xl mb-2 pt-2 pr-4">
          {editorLanguages?.selectedTargetLanguage.targetLanguageName}
        </div>
      </div>
